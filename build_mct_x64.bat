...
rem ---------------------------------------------------------------
rem  If gamepad_serial_x64.lib is missing, build the DLL now
rem  (requires gamepad_serial.cpp to be present)
rem ---------------------------------------------------------------
if not exist "gamepad_serial_x64.lib" (
    echo [INFO] gamepad_serial_x64.lib not found -- building gamepad DLL first...
    echo.
    ...
    cl.exe /nologo /std:c++17 /EHsc /O2 /W3 /MT /DNDEBUG /D_WIN32_WINNT=0x0601 ^
        /D GAMEPAD_SERIAL_EXPORTS /LD gamepad_serial.cpp /Fe:gamepad_serial_x64.dll ^
        /link /DLL /SUBSYSTEM:WINDOWS,6.1 /MACHINE:X64 /OPT:REF /OPT:ICF

    if !ERRORLEVEL! neq 0 (
        echo [FAILED] gamepad DLL build failed.
        pause & exit /b 1
    )
    echo [OK] gamepad_serial_x64.dll + .lib built successfully.
    echo.
)

echo [OK] Boost   : %BOOST_INC%
echo [OK] Gamepad : gamepad_serial_x64.lib found
echo [OK] Target  : x64
echo.

rem ---------------------------------------------------------------
rem  Locate Boost libraries (x64 example names)
rem ---------------------------------------------------------------
set "BOOST_FS_LIB=boost_filesystem-vc145-mt-x64-1_90.lib"
set "BOOST_CHRONO_LIB=boost_chrono-vc145-mt-x64-1_90.lib"
set "BOOST_ATOMIC_LIB=boost_atomic-vc145-mt-x64-1_90.lib"
set "BOOST_DT_LIB=boost_date_time-vc145-mt-x64-1_90.lib"

if not exist "%BOOST_LIB%\!BOOST_FS_LIB!" (
    echo [ERROR] Cannot find !BOOST_FS_LIB! in %BOOST_LIB%
    dir "%BOOST_LIB%\*.lib" /b
    pause & exit /b 1
)
if not exist "%BOOST_LIB%\!BOOST_CHRONO_LIB!" (
    echo [ERROR] Cannot find !BOOST_CHRONO_LIB! in %BOOST_LIB%
    dir "%BOOST_LIB%\*.lib" /b
    pause & exit /b 1
)
if not exist "%BOOST_LIB%\!BOOST_ATOMIC_LIB!" (
    echo [ERROR] Cannot find !BOOST_ATOMIC_LIB! in %BOOST_LIB%
    dir "%BOOST_LIB%\*.lib" /b
    pause & exit /b 1
)
if not exist "%BOOST_LIB%\!BOOST_DT_LIB!" (
    echo [ERROR] Cannot find !BOOST_DT_LIB! in %BOOST_LIB%
    dir "%BOOST_LIB%\*.lib" /b
    pause & exit /b 1
)
...
echo.
echo [..] Compiling MakeshiftCaptureTool_x64.exe ...
echo.
...
cl.exe /nologo /std:c++17 /EHsc /O2 /W1 /MT /GL /DNDEBUG ^
    ...
    /Fe:MakeshiftCaptureTool_x64.exe ^
...
echo.
echo    MakeshiftCaptureTool_x64.exe
echo.
echo  Keep these files together to run:
echo    MakeshiftCaptureTool_x64.exe
echo    gamepad_serial_x64.dll
echo ================================================================
echo.
pause